...
The R script in the "R Scripts" folder can estimate total catch in HMRFS. The total catch estimates can be compared (and validated) with the estimates (including variance) queried from the NOAA Fisheries Marine Recreational Information Program (https://www.fisheries.noaa.gov/data-tools/recreational-fisheries-statistics-queries). The proportion of non-sold catch was also estimated in the R script to compare with the proportion estimates from Ma et al. 2019. When "catch_type" is set as "Non-sold catch', the catch with dispostion "sold/to be sold" is not read into R and the catch estimation is for unsold catch. 
