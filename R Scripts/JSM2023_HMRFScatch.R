...
i1 = read.sas7bdat(file.path(root_dir,"Data/i1_c_add2022f.sas7bdat")) 
observed_catch = read.sas7bdat(file.path(root_dir,"Data/i3_c_add2022f.sas7bdat"))  
unavailable_catch = read.sas7bdat(file.path(root_dir, "Data/i2_c_add2022f.sas7bdat")) 
catch_type <- "Non-sold catch" 
# default is "Non-sold catch", it can be assigned as "Total catch" 

if(catch_type=="Non-sold catch"){
observed_catch <- observed_catch[observed_catch$DISP3 %in% c(3,4,6,7,8,9,0),] # exclude disposition 5 (sold/to be sold), records from 22914 to 19947
unavailable_catch <- unavailable_catch[unavailable_catch$DISPO %in% c(1,2,3,4,6,7,8,9,0),] # exclude disposition of 5, records from 17915 to 15963
}
    ...
    for(s in 1:n_species) {
      for(m in 1:n_modes) {
        for(a in 1:n_areas) {
          ...
          observed_catch_rate = (sum(observed_total_caught[y, w, s, m, a, ]) / num_trips[y, w, m, a]) / mean(anglers_by_trip[y, w, m, a, domain_trips[y, w, m, a, ]]) #the catch rate is not used in the line below but can be used
...
effort_df = read.csv(file.path(root_dir,"Data/mrip_effort_series2022finalized.csv"))
...
total_catch_proportion_sold = 1 - apply(total_catch_num_kept, c(1, 3), sum, na.rm = T) / apply(total_catch_num, c(1, 3), sum, na.rm = T) # [year, species]
...
official_catch = read.csv(file.path(root_dir,"Data/HMRFS catch2022f2.csv")) #changed to f2 on July22, 2025
...
#*** capture these wavely results*** 3=combined area and 2 for Kalman filter
...
# --------------- Section 6: Catch number capture and converting to catch weight and adjustment---------------------

#*********capture original catch and variance by n_year_waves(=20*6)
n_year_waves = n_years * n_waves
# in the second-to-last dimension, index 1 = ocean (> 3 mi), index 2 = ocean (<= 3 mi), and index 3 = aggregated across the two areas; there is no deep 7 catch from inland fishing
# first dimension in these arrays is the sequential waves in order from oldest to most recent
#total_caught_yw = array(0, dim = c(n_year_waves, n_species, n_modes, n_areas, n_dispositions))
...
#*********Adjustments for fishing effort from CHTS and converting catch number to weight

catchwgt = array(0, dim = c(120,7))
catchnum <- as.data.frame(wave_smoothed_total_catch[, , 3, 2])
#catchnum = read.csv("jsmSmoothedWavelynonsold_2022complete.csv")
#df = catchnum[-c(1)]
df = catchnum
#df[42,4]=df[42,4]+4269.557 
df[42,4]=df[42,4] + unaccounted_total_catch[7,4]
#adding catch from shore fishing
modify = read.csv(file.path(root_dir,"Data/Multiplier.csv"))
M1 = modify$M1
M2 = modify$M2
meanwgt = read.csv(file.path(root_dir,"Data/meanweight.csv"))
wgt = meanwgt$lbs
...
dfwgt$FY = modify$FY
dfwgt$CY = modify$CY
write.csv(dfwgt, file.path(root_dir, "Output/jsmSmoothedwavelynonsold_cachweight.csv"))
...
dfvar$FY = modify$FY
dfvar$CY = modify$CY
write.csv(dfvar,file.path(root_dir, "Output/jsmdataforPSE.csv"))
write.csv(as.data.frame(year_totalcatch), file.path(root_dir, "Output/jsmOrig_yearlynonsold.csv"))
write.csv(as.data.frame(yearwave_totalcatch_num), file.path(root_dir, "Output/jsmunsmoothedwavelynonsold_catch.csv"))
