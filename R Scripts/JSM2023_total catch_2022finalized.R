...
library(zoo)
library(KFAS)
setwd("C:/Users/Hongguang.Ma/Documents/Telework/Deep 7 Analyses/Data_Toby")
#Modify to calculate total catch for JSM2023 manuscript May27 and use finalized fishing effort for 2022
#Version Sept 20th to capture non-sold catch and smoothed catch for comparision with previously saved results above shinny
i1 = read.sas7bdat("i1_c_add2022f.sas7bdat", debug = TRUE) # trip header data, 59080 records
observed_catch = read.sas7bdat("i3_c_add2022f.sas7bdat", debug = TRUE) # _ns for non-sold catch, 19947 records
unavailable_catch = read.sas7bdat("i2_c_add2022f.sas7bdat", debug = TRUE) # _ns for non-sold, 15963 records
...
effort_df = read.csv("mrip_effort_series2022finalized.csv")
...
official_catch = read.csv("HMRFS catch2022f.csv")
...
setwd("C:/Users/Hongguang.Ma/Documents/Telework/Deep 7 Analyses/Total catch_jsm")

write.csv(as.data.frame(wave_smoothed_total_catch[, , 3, 2]), file="jsmSmoothedWavelynonsold_2022complete.csv")
write.csv(as.data.frame(cpue_raw2), file="jsmCPUE_raw_2022complete.csv")
write.csv(as.data.frame(unaccounted_total_catch), file="jsmDeep7catch_fromSH.csv")
write.csv(as.data.frame(kalman_smoothed_catch_plot), file="jsmSmoothedYearlynonsold_includeSH.csv")
write.csv(as.data.frame(year_totalcatch), file="jsmOrig_yearlynonsold.csv")
write.csv(as.data.frame(yearwave_totalcatch_num), file ="jsmunsmoothedwavelynonsold_catch.csv")
write.csv(as.data.frame(yearwave_totalcatch_num_var), file ="jsmvariance_bywave.csv")


#setwd("C:/Users/Hongguang.Ma/Documents/Telework/Deep 7 Analyses/Data_final")
...
dfwgt$FY = modify$FY
dfwgt$CY = modify$CY
write.csv(dfwgt, file="jsmSmoothedwavelynonsold_cachweight.csv")

origcatch = read.csv("jsmunsmoothedwavelynonsold_catch.csv")
variance = read.csv("jsmvariance_bywave.csv")
...
dfvar$FY = modify$FY
dfvar$CY = modify$CY
write.csv(dfvar, file="jsmdataforPSE.csv")
